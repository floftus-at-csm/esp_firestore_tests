// I've disabled some of the code in FirebaseFS.h in the firebase library to save memory
#if defined(ESP32)
#include <WiFi.h>
...
void loop() {
  ...
  if (Firebase.ready() && (millis() - dataMillis > 100000 || dataMillis == 0)) {
    dataMillis = millis();
    String documentPath = "instructions/image";
    String mask = "length";

    // if (Firebase.Firestore.getDocument(&fbdo, FIREBASE_PROJECT_ID, "", documentPath.c_str(), mask.c_str())) {
    //   Serial.printf("ok\n%s\n\n", fbdo.payload().c_str());
    //   // dataLength = getDataFromFirestore(payload, "fields/length/integerValue");
    //   FirebaseJson payload;
    //   payload.setJsonData(fbdo.payload().c_str());
    //   FirebaseJsonData jsonData;
    //   payload.get(jsonData, "fields/length/integerValue", true);
    //   Serial.println(jsonData.intValue);
    //   dataLength = jsonData.intValue;
    // }

    mask = "data";

    Serial.print("Get a document 2... ");
    ...
    if (Firebase.Firestore.getDocument(&fbdo, FIREBASE_PROJECT_ID, "", documentPath.c_str(), mask.c_str())) {
      ...
      String imageBase64 = jsonData.stringValue;
      char charBuf[imageBase64.length()];                      // creating array for characters from dataURL/bs
      imageBase64.toCharArray(charBuf, imageBase64.length());  // converting string to char array I think this outputs char* not just char
      unsigned char *newCharArray = (unsigned char *)charBuf;
      // decoding base64 image
      unsigned char decoded_text[dataLength];
      int decoded_length = decode_base64(newCharArray, dataLength, decoded_text);
      Serial.print("Decoded Text: ");
      Serial.println((char *)decoded_text);
...
int getIntFromFirestore(FirebaseJson startingJson, String pathToInt) {
}
